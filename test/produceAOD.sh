...
inputFilePath_ggH_htt="\/hdfs\/local\/tolange\/step3\/val\/71\/"
...
rm $configFile
sed_command="s/##inputFilePath/inputFilePath/; s/\$inputFilePath/$inputFilePath_ggH_htt/; s/##mode/mode/; s/\$mode/$mode/; s/#process.tallinnTaus.mode = cms.string('regression')/process.tallinnTaus.mode = cms.string('$cmode')/"
...
rm $configFile
sed_command="s/##inputFilePath/inputFilePath/; s/\$inputFilePath/$inputFilePath_qcd/; s/##mode/mode/; s/\$mode/$mode/; s/#process.tallinnTaus.mode = cms.string('regression')/process.tallinnTaus.mode = cms.string('cmode')/"
sed "$sed_command" produceTallinnTaus_AOD_cfg.py >& $configFile
